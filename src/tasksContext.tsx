...
const TasksProvider = ({ children }: { children: ReactNode }) => {
  ...
  const getTimeSpendInMs = (task: TaskType): number => {
    if (task.isTaskStarted) {
      ...
      if (task.isCompleted) {
        ...
        // If the user is working on the task
  ...
  const startBreakTimer = (task: TaskType, shortBreak?: boolean) => {
    ...
    if (task.isCompleted) {
      ...
      tasks[taskIndex].totalTimeSpend =
        endDateTime - startDateTime - totalTimeBreak;
    }
  ...
  const stopBreakTimer = (task: TaskType) => {
    ...
    tasks[taskIndex].totalTimeBreak =
      totalTimeBreak + (endDateBreakTime - startDateBreakTime);
